    ...
    <div class="card">
        <div class="card-image">
            ...
            <form class="formObra">
                ...
                @if(Auth::user())
                    @if($obra->likes->where('user_id', Auth::user()->id)->count() == 0)
                        <a data-position="right" data-delay="0" data-tooltip="Like"
                           class="btn-floating halfway-fab btn-large waves-effect waves-light red likeObra"><i
                ...
                @else
                    <a data-position="right" data-delay="0" data-tooltip="Like"
                       class="btn-floating halfway-fab btn-large waves-effect waves-light noLogin grey"><i
                                class="material-icons tiny">thumb_up</i></a>
                @endif
            </form>
        </div>
        ...
        <div class="card-action">
            <a style="font-weight: bold" class="black-text">A <span
                        class="numLikes">{{$obra->likes->count()}}</span> les gusta esto.</a>
        </div>
    </div>
</div>
